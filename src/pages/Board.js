...
const Board = () => {
    ...
    const findById = useCallback(async () => {
        try {
            const response = await axios.get(`https://board-app-back.store:8443/boards/${id}`, {
                headers: {
    ...
    const deleteById = useCallback(async () => {
        try {
            const resonse = await axios.delete(`https://board-app-back.store:8443/boards/${id}`, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
    ...
    const modify = useCallback(async (formData) => {
        try {
            const response = await axios.patch('https://board-app-back.store:8443/boards', formData, {
                headers: {
                    Authorization: `Bearer ${sessionStorage.getItem('ACCESS_TOKEN')}`
                }
            });
